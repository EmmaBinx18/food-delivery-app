...
router.get('/user/:userId', (req, res) => {
    ...
    try {
        db.executeStoredProcedure(sp.GET_USER_ADDRESS, { userId: req.params.userId }, (data) => {
            return res.status(200).send(data);
        });
    }
    catch (error) {
...
router.post('/user/:userId', (req, res) => {
    ...
    try {
        db.executeStoredProcedure(sp.DELINK_USER_ADDRESS, { userId: req.params.userId, addressId: req.body.params }, (data) => {
            return res.status(200).send(data);
        });
    }
    catch (error) {
