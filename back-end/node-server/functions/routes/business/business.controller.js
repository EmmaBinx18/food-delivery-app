...
router.get('/user/:userId', (req, res) => {
    ...
    try {
        db.executeStoredProcedure(sp.GET_BUSINESS_USER, { userId: req.params.userId }, (data) => {
            return res.status(200).send(data);
        });
    }
    catch (error) {
...
router.get('/category/:categoryId', (req, res) => {
    ...
    try {
        db.executeStoredProcedure(sp.GET_BUSINESS_CATEGORY, { categoryId: req.params.categoryId }, (data) => {
            return res.status(200).send(data);
        });
    }
    catch (error) {
...
router.get('/operationalStatus/:operationalStatusId', (req, res) => {
    ...
    try {
        db.executeStoredProcedure(sp.GET_OPERATIONAL_STATUS, { operationalStatusId: req.params.operationalStatusId }, (data) => {
            return res.status(200).send(data);
        });
    }
    catch (error) {
